...
main: 		
		...
		# A main coordena as entradas e saídas do programa
		# Explicar como a documentação foi organizada
...
escolhe_operacao:		
		...
		# t3: guarda o valor do caractere '+', '-', '*' ou '/' para comparar depois
		# t2: guarda o valor do caractere lido do usuário
		#
		# ----------------------------
...
somar:
		...
		jal inserir_resultado	# Insere o valor na estrutura que armazena os resultados
...
desempilhar_resultado:
		...
		# Verifica se a lista está vazia
	    	beqz t1, pilha_vazia	# se o t1 (endereço do nó cabeça) == 0, 
...
pilha_vazia:
		# --- CASO DE PILHA VAZIA ---
		# Esta função é chamada quando a pilha ligada está vazia (head == NULL).
		...
		# --------------------------------------------------------
	    	...
	    	li a0, 0	# a0 recebe 0, pois não há valor para desempilhar
	    	li a1, 0	# a1 recebe 0, indicando falha (pilha vazia)
	    	jr ra		# retorna à função chamadora 
...
undo:
		...
		# Ela desempilha o resultado anterior e o restaura para que a operação anterior seja revertida.
		#
		# Registradores usados:
		# a0 – valor do desempilhamento (não utilizado diretamente nesta função)
		...
		# Chamando a função desempilhar_resultado para obter o valor anterior
 	    	call desempilhar_resultado
 	    	
 	    	# Verifica se a flag a1 é igual a 0, o que indica que a pilha estava vazia
	    	beqz a1, sem_resultado_para_restaurar  # se flag == 0
	
	    	# Carrega o ponteiro para o topo da pilha em t0
	    	la t0, head
	    	lw t1, 0(t0)       			# t1 = novo topo da pilha (novo nó após desempilhar)
	    	beqz t1, sem_resultado_para_restaurar	# Verifica se o topo é nulo (pilha vazia)
	
		# Carrega o valor armazenado no novo topo da pilha em t2 (valor anterior)
	    	lw t2, 0(t1)      
